...
public class AddressService : ServiceBase<AddressService>
{
    ...
    public nint KineDriverPosition { get; private set; }
    public nint KineDriverRotation { get; private set; }
    public nint KineDriverScale { get; private set; }
    ...
    private async Task Scan()
    {
        ...
        var tasks = new List<Task>
        {
            ...
            // Disabled for Patch 7.4 (Anamnesis moved to Detour Hooks, AQuestReborn uses Brio)
            ...
            this.GetAddressFromSignature("PlayerTargetSystem", "48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 3B C6 0F 95 C0", 0, (p) => { PlayerTargetSystem = p; }),
            this.GetAddressFromTextSignature("TimeAsm", "48 89 87 ?? ?? ?? ?? 48 69 C0", (p) => TimeAsm = p),
            this.GetAddressFromSignature("Framework", "48 8B 1D ?? ?? ?? ?? 8B 7C 24", 0, (p) => Framework = MemoryService.ReadPtr(p)),
            // Disabled for Patch 7.4 (Anamnesis moved to Detour Hooks)
